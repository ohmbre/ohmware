...
static void remain_on_channel_expire(struct timer_list *data)
{
	struct mwl_priv *priv = from_timer(priv, data, roc.roc_timer);
...
void timer_routine(struct timer_list *data)
{
        
        struct mwl_priv *priv = from_timer(priv, data, period_timer); //(struct mwl_priv *)data;
...
void ds_routine(struct timer_list *data)
{
  struct mwl_priv *priv = from_timer(priv, data, ds_timer);
	struct ieee80211_hw *hw = priv->hw;
